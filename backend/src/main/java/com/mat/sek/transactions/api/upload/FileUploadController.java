...
public class FileUploadController {
    ...
    public FileUploadResponse uploadFile(@PathVariable CsvFileType type, @RequestParam("file") MultipartFile file) throws IOException {
        ...
        Resource storedResource = storageService.store(type.name(), file);
        uploadHandlerService.handle(type, storedResource.getFile().toPath());
        String fileName = Objects.requireNonNull(storedResource.getFilename());
